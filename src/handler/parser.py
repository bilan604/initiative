...
def getText(e: str):
    ...
    for i in range(len(e)):
        if e[i] == "<":
            ...
            if not stack:
                print("the HTML code has uneven opening and closing elements")
                return ""
            stack.pop()
        else:
            if not stack:
...
def getTag(element):
    """
    returns the opening tag for a function
    """
    idx = element.find(">")
    if not idx:
        return ""
...
def getProperties(tag: str):
    """
    This function takes the opening tag for a HTML element
    and returns a dictionary of the element's properties as key value pairs.
    """
    if not tag:
...
def get_xpath_by_element(element):
    # returns a selenium xpath given an HTML element
    if not element:
...
def matchOptionStrings(answer: str, optionStrings: list[str]):
    """
    answer: Your answer for some given question.
    optionStrings: the labels for answer options to a user input question online

    returns: a tuple containing the option and the options index most similar to the desired answer
    """
